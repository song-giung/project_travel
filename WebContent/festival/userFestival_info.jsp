...
ul{
	...
	#map_li{
		border: 1px solid silver; border-radius: 5px; height: 450px
	}
	.navBulletsWrapper{
		display: block;
	}
...
geocoder.addressSearch(address, function(result, status) {
	...
	$(document).on("click","#up",(function(){
		 ...
		 if(upcheck=="0" && downcheck=="0"){
			 ...
			 if (confirm("해당 축제를 추천하겠습니까?") == true){    
				   
				    
				    
				    $.ajax({
						
						data : {"festival_no":no,"user_no":user_no},
						url:"./festival_upadd.fs",
						success:function(data){
		 ...
		 if(upcheck=="1" && downcheck=="0"){
			 ...
			 if(confirm("해당 축제 추천을 취소하겠습니까?") == true){
				 $('#upcheck').val("0");
				 $('#up').val("추천!");
	...
	function down(){
	 ...
	 if(upcheck=="0" && downcheck=="0"){
		 ...
		 if (confirm("해당 축제를 비추천하겠습니까?") == true){    
			    $('#downcheck').val("1");
			    $('#down').val("비추천중!");
...
<body>
	...
	<input type="hidden" id="address" name="address" value="${festival.festival_address }">
	<input type="hidden" id="name" name="name" value="${festival.festival_name }">
	<input type="hidden" id="no" name="no" value="${festival.festival_no }">
	<input type="hidden" id="user_no" name="user_no" value="${sessionScope.user_no }">
	<h1 style="text-align: center;">${festival.festival_name }</h1>
	...
	<div style="display: block; width: 100%; margin: 0 auto;" >
		...
		<div id="sliderFrame" style="display:inline; width: 800px;" >
				<div id="slider" style="width: 700px; float:left;">
					<c:forEach var="img" items="${imglist }">
						<img src="./festivalUpload/${img.festival_img_name }"width="700px" height="306px"  >
					</c:forEach>
				</div>
		</div>
		</c:if>
		...
		1.주소지 :${festival.festival_address }<br><br>
		<br><span>2.축제 소개글</span><br>${festival.festival_content }
		</div>
			<input type="button" id="pocketplus" value="축제 장바구니에 추가하기" id="pocketplus" style="align-items: center;">
		</div>
	</div>
	</li>
	<div id="updown">
	...
	<li id="map_li">
	<div id="map" style="width:90%;height:450px; margin: 0 auto">
	</div>
	</li>

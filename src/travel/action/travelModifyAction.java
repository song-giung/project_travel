...
public class travelModifyAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			int no=Integer.parseInt(multi.getParameter("travel_no"));
			...
			tdao.travel_modify(t);
			
			
			
			//여행지 타입.
			t_type.setTravel_no(no);
			tdao.deleteTravelType(t_type);
			System.out.println("~!!!!!!");
			for(int i=0;i<user_type.length;i++) {
				t_type.setTravel_type(Integer.parseInt(user_type[i]));
				tdao.modifyTravelType(t_type);
			}
			...
			for(int i=1;i<5;i++) {
				...
				if(fileName!=null) {
					//이미지가 변경될경우 전부삭제하고 다시 삽입
					t_img.setTravel_no(no);
					t_imgdao.travel_img_delete(t_img);
